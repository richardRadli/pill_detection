...
def plot_efd(filename, coeffs, locus=(0.0, 0.0), image=None, contour=None, n=300):
    ...
    for n in range(coeffs.shape[0]):
        ...
        if image is not None:
            # A background image of shape [rows, cols] gets transposed by imshow so that the first dimension is vertical
            # and the second dimension is horizontal. This implies swapping the x and y axes when plotting a curve.
    ...
    # plt.show()
    plt.tight_layout()
    plt.savefig(f"C:/Users/ricsi/Desktop/fourier/{filename}")
    plt.close()
    gc.collect()
...
def elliptic_fourier(image, segmented_image, normalize, filename=None):
    contours, _ = cv2.findContours(segmented_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    largest_contour = max(contours, key=cv2.contourArea)
    largest_contour = np.squeeze(largest_contour)

    if normalize:
        coefficients = pyefd.elliptic_fourier_descriptors(largest_contour, order=5, normalize=True)
        ...
        coefficients = pyefd.elliptic_fourier_descriptors(largest_contour, order=5, normalize=False)

        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        rotated_image = cv2.transpose(image)
        plot_efd(filename, coefficients, locus, rotated_image, largest_contour)
...
def plot_euclidiean_distances(class_averages):
    ...
    # Show the plot
    plt.tight_layout()
    plt.savefig("C:/Users/ricsi/Desktop/euclidean_distances.png", dpi=300)
    plt.close()
...
def compare_distances(class_averages):
    # Assuming you have a new image and have extracted its Fourier descriptor
    new_image_path = \
        "C:/Users/ricsi/Documents/project/storage/IVM/datasets/nih/ref/10144060615/W5VCARJ2K86QFNBMK6E1!SIZIBD7CY.JPG"
    new_image_original = cv2.imread(new_image_path, 1)
    new_image = cv2.cvtColor(new_image_original, cv2.COLOR_BGR2GRAY)
    new_segmented_image = preprocess_image(new_image)
    new_fourier_descriptor = elliptic_fourier(image=new_image_original,
    ...
    for class_label, class_vector in class_averages.items():
        distance = calculate_distance(new_fourier_descriptor, class_vector)
    ...
    print(f"\nThe closest class to the new image is: {closest_class}")


def main():
    file_path = "C:/Users/ricsi/Desktop/collected_images_by_shape_nih"
    class_averages = {}

    for root, dirs, files in os.walk(file_path):
        for class_label in dirs:
            class_coefficients = []  # To store norm_fourier_coeff for each image in the class
            ...
            for file in os.listdir(class_path):
                ...
                try:
                    ...
                    norm_fourier_coeff = elliptic_fourier(image=image_original,
    ...
    # plot_euclidiean_distances(class_averages)
    compare_distances(class_averages)
