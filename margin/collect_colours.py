...
def plot_image(image, width, height, x1, y1, x2, y2, filename):
    cv2.rectangle(image, (x1, y1), (x1 + width, y1 + height), (255, 255, 255), 2)
    cv2.rectangle(image, (x2, y2), (x2 + width, y2 + height), (233, 233, 233), 2)
            ...
            if idx == 1:
                ...
                for row in sheet.iter_rows(min_row=2, values_only=True):
                    ...
                    if ndc11 == class_name:
                        ...
                        filename = f"{ndc11}_{colour}_{shape}.jpg"
                        plot_image(image, width, height, x1, y1, x2, y2, filename)
                        shape_class_colour[class_name] = (rgb1, rgb2)

    print(shape_class_colour)
    with open("C:/Users/ricsi/Desktop/color_pills/colors.json", "w") as json_file:
        json.dump(shape_class_colour, json_file, cls=NumpyEncoder)
