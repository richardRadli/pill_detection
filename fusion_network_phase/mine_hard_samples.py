...
def get_hardest_samples():
    ...
    latest_hard_samples_contour = (
        find_latest_file_in_latest_directory(
            path=hard_sample_paths.get("hard_sample").get("Contour")
        )
    )

    latest_hard_samples_lbp = (
        find_latest_file_in_latest_directory(
            path=hard_sample_paths.get("hard_sample").get("LBP")
        )
    )

    latest_hard_samples_rgb = (
        find_latest_file_in_latest_directory(
            path=hard_sample_paths.get("hard_sample").get("RGB")
        )
    )

    latest_hard_samples_texture = (
        find_latest_file_in_latest_directory(
            path=hard_sample_paths.get("hard_sample").get("Texture")
        )
    )
