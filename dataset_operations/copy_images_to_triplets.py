...
def copy_images(dataset, src_subset, dst_subset, stream):
    ...
    src_path = (
        dataset_images_path_selector(dataset).get("src_stream_images").get(src_subset).get(f"stream_images_{stream}")
    )
    dst_path = (
        dataset_images_path_selector(dataset).get("dst_stream_images").get(dst_subset)
    )
    dst_path = (
        str(os.path.join(dst_path, stream))
    )
    ...
    pill_class = (
        train_pill_classes) if dst_subset in ["stream_images_anchor", "stream_images_pos_neg"] else test_pill_classes
